...
describe('AuthService.loginWithGoogle — flujo de Parent con invitación', () => {
  ...
  beforeEach(async () => {
    ...
    prisma = {
      ...
      // findFirst por defecto resuelve null (nadie es "padre de retorno" a
      // menos que un test lo pise explícitamente) — así el resto de los
      // tests, que ejercitan el flujo de invitación de siempre, no cambian.
  ...
  it('INVITATION_INVALID_ROLE — rechaza si parentId es null', async () => {
    prisma.schoolInvitation.findUnique.mockResolvedValue({ ...validInvitation, parentId: null });
    await expect(
      service.loginWithGoogle({ idToken: 'x', invitationToken: 't' } as any),
    ).rejects.toMatchObject({ message: 'INVITATION_INVALID_ROLE' });
  });
